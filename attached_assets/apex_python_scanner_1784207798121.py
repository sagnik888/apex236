...
class ApexConfig:
    ...
    def validate(self) -> None:
        ...
        if self.max_consecutive_losses < 1 or self.circuit_pause_bars < 0:
...
def regime_code(frame: pd.DataFrame) -> pd.Series:
    ...
    vol = np.select([atr_ratio > 1.25, atr_ratio < 0.85], [10, 20], default=0)
...
def build_feature_frame(
    ...
    for length in (5, 9, 13, 21, 50, 200):
        f[f"ema{length}"] = ema(df["close"], length)
    ...
    f["rsi_5m_prev"] = previous_closed_mtf(
        df, "5min", lambda x: rsi(x["close"], 14),
        timestamps_are_bar_close=config.timestamps_are_bar_close,
    )
    f["ema9_15m_prev"] = previous_closed_mtf(
        df, "15min", lambda x: ema(x["close"], 9),
        timestamps_are_bar_close=config.timestamps_are_bar_close,
    )
    f["ema21_15m_prev"] = previous_closed_mtf(
        df, "15min", lambda x: ema(x["close"], 21),
        timestamps_are_bar_close=config.timestamps_are_bar_close,
    )
    ...
    # MTF regime codes, mapped from the previous fully closed source bar.
    for label, rule in (("1m", "1min"), ("15m", "15min"), ("1h", "60min"), ("4h", "240min"), ("1d", "1D")):
        f[f"regime_{label}"] = previous_closed_mtf(
            df, rule, regime_code,
            timestamps_are_bar_close=config.timestamps_are_bar_close,
        )
    ...
    adx_den = max(1.0, 25.0 - config.min_adx)
    bull_adx = np.where(
        (f["adx"] >= config.min_adx) & (f["di_plus"] > f["di_minus"]),
        np.where(f["adx"] > 25.0, 20.0, 10.0 + (f["adx"] - config.min_adx) * (10.0 / adx_den)),
        0.0,
    )
    bear_adx = np.where(
    ...
    f["bull_score"] = bull_raw.clip(upper=100.0)
    f["bear_score"] = bear_raw.clip(upper=100.0)
    ...
    f["setup_breakout"] = (df["high"] > high20_prev) & f["volume_spike"] & (df["close"] > df["open"])
    f["setup_breakdown"] = (df["low"] < low20_prev) & f["volume_spike"] & (df["close"] < df["open"])
    f["setup_pull_buy"] = f["bull_trend"] & (df["close"] < f["ema21"]) & (df["close"] > f["ema50"]) & f["rsi"].between(40.0, 60.0, inclusive="neither")
    f["setup_pull_sell"] = f["bear_trend"] & (df["close"] > f["ema21"]) & (df["close"] < f["ema50"]) & f["rsi"].between(40.0, 60.0, inclusive="neither")
    f["setup_momentum_buy"] = crossover(f["macd"], f["macd_signal"]) & f["volume_spike"] & (df["close"] > f["ema21"])
    f["setup_momentum_sell"] = crossunder(f["macd"], f["macd_signal"]) & f["volume_spike"] & (df["close"] < f["ema21"])
    f["setup_reversal_buy"] = (f["rsi"] < 35.0) & (df["close"] > df["open"]) & (df["close"] > df["high"].shift(1)) & (f["macd"] > f["macd_signal"])
    f["setup_reversal_sell"] = (f["rsi"] > 65.0) & (df["close"] < df["open"]) & (df["close"] < df["low"].shift(1)) & (f["macd"] < f["macd_signal"])
    f["anchor_low_3"] = df["low"].rolling(3, min_periods=3).min()
    f["anchor_high_3"] = df["high"].rolling(3, min_periods=3).max()
    return f
...
def calculate_stops(
    ...
    use_fixed = not (bool(row["high_volatility"]) or bool(row["is_trending"])) and config.fixed_sl_pct > 0.0
    if use_fixed:
    ...
    def run_symbol(
        ...
        *,
        ...
        numeric_cols = [
            ...
            "live_pnl_pct", "live_rr", "realized_pnl_pct", "realized_pnl_r",
        ]
        ...
        for i in range(len(f)):
            ...
            if active is not None:
                ...
                if stop_event is not None:
                    ...
                    if active.direction == "LONG":
                        ...
                        met += int(bool(row["rsi"] < 45.0 and row["rsi"] < f["rsi"].iloc[i - 1])) if i > 0 else 0
                        ...
                        met += int(bool(row["close"] > row["ema13"]))
                        met += int(bool(row["rsi"] > 55.0 and row["rsi"] > f["rsi"].iloc[i - 1])) if i > 0 else 0
                        threshold = 3 if active.t1_hit else 2
                        if met >= threshold:
                            active.exit_confirmation_count += 1
                        elif row["close"] < row["ema9"]:
                    ...
                    if momentum_exit:
                        exit_price, exit_reason = float(row["close"]), "Momentum Exit"
                ...
                if exit_price is not None:
                    ...
                    if pnl_pct > 0.0:
                        ...
                        if consecutive_losses >= cfg.max_consecutive_losses:
                            pause_until_bar = i + cfg.circuit_pause_bars
                    active = None
                    last_signal_state = 0
            ...
            can_signal = (i - last_signal_bar) >= cfg.signal_cooldown and i > pause_until_bar
            ...
            if active is not None:
                risk = abs(active.entry_price - active.sl1)
                live_pnl = ((float(row["close"]) - active.entry_price) / active.entry_price * 100.0) if active.direction == "LONG" else ((active.entry_price - float(row["close"])) / active.entry_price * 100.0)
